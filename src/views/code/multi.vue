<template>
    <div class="form-container">
        ...
        <el-form ref="code" :model="code" label-width="100px" :inline="true">
            <el-tabs v-model="current" type="card" closed @tab-remove="removeTab">
                <el-tab-pane v-for="(item,index) in codes" :key="index" :value="index" :label="String(item.index)" :name="String(item.index+1)">
                    <el-form-item v-if="!is_add" label="邀请码" prop="code" style="display: block;">
                        <el-input type="text" v-model="item.code.code" readonly disabled autocomplete="off" style="width: 260px;"></el-input>
                    </el-form-item>
                    <el-form-item v-if="is_add" label="生成数量" style="display: block;">
                        <el-input type="number" v-model="count"></el-input>
                    </el-form-item>
                    <el-form-item label="过期日期">
                        <el-date-picker format="yyyy-MM-dd" value-format="yyyy-MM-dd" v-model="item.code.date_expired" type="date" placeholder="选择日期"></el-date-picker>
                    </el-form-item>
                    <el-form-item label="关联作者" style="display: block;">
                        <el-select
                            v-model="item.code.authors"
                            multiple
                            filterable
                            placeholder="关联作者"
                            style="width: 500px"
...
<script lang="ts">
    ...
    import { Form,FormItem,Input,Button,DatePicker,Select,Option,Tabs,TabPane } from 'element-ui'
    ...
    export default class CodeForm extends Vue{
        ...
        @Provide() codes:Array<{index:any,code:TypeCode|any}>=[{index:0,code:{}},{index:'添加',code:{}}]
